...
if numel(dir([dscpath '/ep2d_perf/'])) <=2
    ...
    for i = 1:totaltimes
        if i ~= 1 %if it's not the first volume... read in the nii file
            num = num2str(times(i), '%.0f'); %time point
        ...
        for j = 1:totalslices
            if i == 1 %first timepoint is registered to itself, so just copy old to new!
